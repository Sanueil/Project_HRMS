...
function getFilteredWorkTimeData($db, $date)
{
    ...
    $query = "SELECT maNhanVien, COUNT(maNhanVien) as numCheckIns FROM cham_cong WHERE DATE(thoiGianChamCong) = ? GROUP BY maNhanVien";
    ...
    $workTimeData = array('employees' => array(), 'numCheckIns' => array());

    // Kiểm tra kết quả truy vấn và lưu dữ liệu vào mảng
    if ($result->num_rows > 0) {
        while ($row = $result->fetch_assoc()) {
            $workTimeData['employees'][] = $row['maNhanVien'];
            $workTimeData['numCheckIns'][] = $row['numCheckIns'];
        }
    }
